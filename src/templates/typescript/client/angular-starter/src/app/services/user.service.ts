...
// =====> StatusCodes
import { badReqCode, unAuthorizedCode } from '../utils/statusCodes';
  ...
  public async refresh(): Promise<boolean> {
    ...
    await axios({ method, url, headers })
      ...
      .catch(err => {
        ...
        if (
          (data.status === badReqCode && data.data.error === 'invalid_grant') ||
          data.status === unAuthorizedCode
        )
          this.logOut();
      });
