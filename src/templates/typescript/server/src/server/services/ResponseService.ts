...
export const msg = {
  ...
  ok: (
    ...
    note(statusCodes.OK, data, endPoint, method, requestFrom, params, text),
  ...
  created: (
    ...
    note(
      statusCodes.CREATED, data, endPoint, method, requestFrom, params, text
    ),
...
export const respond = (
  ...
  const response =
    (data && typeof data === 'object' && data.status && data.statusText)
      ? note(data.status, data.data, responseUrl, method, referer, query, text)
      : note(status, data, responseUrl, method, referer, query, text);
